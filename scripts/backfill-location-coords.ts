...
async function main() {
  ...
  if (!company) { console.error("Company not found - nothing changed."); process.exit(1); }

  const { data: locations, error: lErr } = await db
    .from("locations").select("id, name, lat, lng").eq("company_id", company.id);
  if (lErr) throw new Error(lErr.message);

  let updated = 0;
  for (const loc of locations ?? []) {
    const target = COORDS[loc.name];
    if (!target) { console.log("skip   " + loc.name); continue; }
    const [lat, lng, radius] = target;
    let error = (await db.from("locations")
      .update({ lat, lng, geofence_radius_m: radius, geofence_enabled: true })
      .eq("id", loc.id)).error;
    if (error && /geofence/.test(error.message)) {
      console.warn("note   migration 0004 missing - lat/lng only for " + loc.name);
      error = (await db.from("locations").update({ lat, lng }).eq("id", loc.id)).error;
    }
    if (error) throw new Error(loc.name + ": " + error.message);
    console.log("set    " + loc.name + " -> " + lat + ", " + lng + " (r=" + radius + "m)");
    updated++;
  }
