...
export function systemPrompt(input: {
  ...
  return [
    ...
    "- Never invent a name, a count, a time or an identifier. Never fill a gap with a plausible guess.",
    "- Distinguish clearly between what the data says and what you are inferring from it.",
    "- Be brief. A manager scanning this on a phone wants numbers and names, not paragraphs.",
    ...
    "",
    "FOLLOW-UPS",
    "- When the manager says 'the first one' or 'that shift', use the identifiers from the tool results earlier in this conversation. Never guess an identifier.",
  ]
    .filter(Boolean)
    .join("\n");
}
